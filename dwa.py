...
class LegacyDWAConfig:
    ...
    """
    max_speed: float = 0.2  # 调高最大线速度，配合真实小车1m/s级别 0.3
    min_speed: float = 0.088  # 默认禁倒车（如需倒车可设为负）。 0.03
    max_yaw_rate: float = 80.0 * math.pi / 180.0  # 提升最大角速度以增强转弯响应
    max_accel: float = 0.2  # 降低加速度，让运动更平滑(m/s^2)。直接影响刹车距离：d≈v^2/(2a)。过小会显得“刹不住”。
    max_delta_yaw_rate: float = 150.0 * math.pi / 180.0  # 角速度变化率上限，加大w窗口对急转更友好。
    v_resolution: float = 0.06  # 速度采样步长。越小越细但更慢；调整0.05->0.06降低采样数。
    yaw_rate_resolution: float = 0.5 * math.pi / 180.0  # 角速度采样步长。调整0.5°->8°降低采样数。
    ...
    turn_speed_threshold: float = 10.0 * math.pi / 180.0  # 当角速度超过该阈值时限制线速度。
    turn_speed_limit: float = 0.08  # 角速度过大时允许的最大线速度。
...
class LegacyDWAPlanner:
    ...
    def plan(self, state: np.ndarray, goal: Tuple[float, float], obstacles: np.ndarray, path_hint: np.ndarray | None = None):
        ...
        if not self.cfg.allow_reverse and sm_v < 0:
            sm_v = 0.0
        self.override_last_command(float(sm_v), float(sm_w), scaled=False)
        ...
        if self.cfg.debug:
            ...
            print(f"[DWA] v={sv:.2f} w={sw:.2f} cost={best_cost:.3f}")
        # 全局步计数（用于启动阶段前进优先策略）
        self._global_step += 1
...
class DWAPlanner:
    ...
    def _print_debug(self, command: Tuple[float, float], cost: float, components: Optional[dict]) -> None:
        print(f"[DWA] v={command[0]:.2f} w={command[1]:.2f} cost={cost:.3f}")
